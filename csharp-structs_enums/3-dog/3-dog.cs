using System;

/// <summary>
/// Represents the rating scale for dogs
/// </summary>
enum Rating
{
    /// <summary>
    /// Represents a good rating
    /// </summary>
    Good,
    /// <summary>
    /// Represents a great rating
    /// </summary>
    Great,
    /// <summary>
    /// Represents an excellent rating
    /// </summary>
    Excellent
}

/// <summary>
/// Represents a dog with its basic information and rating
/// </summary>
struct Dog
{
    /// <summary>
    /// The name of the dog
    /// </summary>
    public string name;
    /// <summary>
    /// The age of the dog in years
    /// </summary>
    public float age;
    /// <summary>
    /// The owner of the dog
    /// </summary>
    public string owner;
    /// <summary>
    /// The rating of the dog
    /// </summary>
    public Rating rating;

    /// <summary>
    /// Initializes a new instance of the Dog struct
    /// </summary>
    /// <param name="name">The name of the dog</param>
    /// <param name="age">The age of the dog in years</param>
    /// <param name="owner">The owner of the dog</param>
    /// <param name="rating">The rating of the dog</param>
    public Dog(string name, float age, string owner, Rating rating)
    {
        this.name = name;
        this.age = age;
        this.owner = owner;
        this.rating = rating;
    }

    /// <summary>
    /// Returns a string representation of the dog with all its information
    /// </summary>
    /// <returns>A formatted string containing the dog's details</returns>
    public override string ToString()
    {
        return string.Format("Dog Name: {0}\nAge: {1}\nOwner: {2}\nRating: {3}", name, age, owner, rating);
    }
}